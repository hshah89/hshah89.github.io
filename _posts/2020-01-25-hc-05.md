...
Let go of the button and type in `AT+RESET` this will reboot the device. At this point, the device should be blinking rapidly
...
OK
```
Let go of the button and type in `AT+RESET` this will reboot the device. After a few seconds, the two devices should pair up. This will be indicated by unison repeated blinking every 1 second.
